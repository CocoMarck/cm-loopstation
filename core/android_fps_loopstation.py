...
import pathlib
ANDROID_BASE_PATH = primary_external_storage_path()
ANDROID_MUSIC_PATH = pathlib.Path( str(ANDROID_BASE_PATH), "Music" )
...
class FPSLoopstation():
    def __init__(self):
        ...
        **Boleanos para determinar posición tempo actual**
        ...
        # Grabador
        self.microphone_recorder = AndroidMicrophoneRecorder()
    ...
    def save_track(self, track_id=None, path=str, loop=True, sample=False, length_in_fps=None):
        '''
        ...
        if limit_reached or update_track:
            ...
            length_in_seconds = sound.length
            if not length_in_fps == None:
                length_in_seconds = length_in_fps/self.fps
            ...
            self.dict_track.update(
                {
                    track_id: {
                        ...
                        "sample": sample,
                        "focus": False,
                        "bars": self.get_seconds_to_bars( length_in_seconds ),
                        "length_in_fps": length_in_seconds*self.fps,
                        "count_fps": 0
                    }
                }
            )
    ...
    def track_recording(self, dict_timer_signal):
        ...
        if (
            ...
            self.microphone_recorder.output_filename = (
                ANDROID_MUSIC_PATH.joinpath(sound_name )
            )
        ...
        if (
            ...
            dict_save_track_signals = self.save_track(
                ...
                sample=False, loop=True, length_in_fps=self.recorder_count_fps
            )
    ...
    def configure_and_get_beat_sounds(self):
        ...
        for path in TEMPO_FILES:
            ...
            self.logging.log( message=f"{sound.length}" )
            self.beat_sounds.append( self.get_sound(path) )
